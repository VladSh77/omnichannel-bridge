...
class OmniBridge(models.AbstractModel):
    ...
    def _omni_meta_credentials(self):
        ...
        if not page_token:
            page_token = ICP.get_param(
                "omnichannel_bridge.meta_page_access_token", ""
            ).strip()
        if not app_secret:
    ...
    def _omni_whatsapp_credentials(self):
        ...
        if not app_secret:
            app_secret = (
                ICP.get_param("omnichannel_bridge.whatsapp_app_secret", "").strip()
                or ICP.get_param("omnichannel_bridge.meta_app_secret", "").strip()
            )
        phone_number_id = (
            ICP.get_param("omnichannel_bridge.whatsapp_phone_number_id", "") or ""
        ).strip()
    ...
    def _omni_viber_credentials(self):
        ...
        if not webhook_secret:
            webhook_secret = ICP.get_param(
                "omnichannel_bridge.viber_webhook_secret", ""
            ).strip()
    ...
    def _omni_verify_viber_signature(self, raw_body, headers):
        ...
        got = (
            ...
            or headers.get("x-viber-content-signature")
            or ""
        )
        expected = hmac.new(
    ...
    def _omni_deliver_inbound(
        ...
        label = _("[%(p)s] %(name)s") % {
            "p": dict(self.env["omni.integration"]._selection_providers()).get(
                provider, provider
            ),
        ...
        if (
            channel.omni_bot_paused
            and channel.omni_bot_pause_reason == "manager_session_active"
        ):
        ...
        if provider == "telegram":
            delay = 0
        job = (
            self.env["omni.ai.job"]
            .sudo()
            .omni_enqueue_autoreply(
                channel=channel,
                partner=partner,
                text=text,
                provider=provider,
        ...
        if provider == "telegram" and job:
            try:
                ...
                _logger.exception(
                    "Immediate Telegram AI run failed, cron fallback will retry"
                )
        return partner
    ...
    def _omni_maybe_create_crm_lead(self, partner, provider):
        ...
        ICP = self.env["ir.config_parameter"].sudo()
        if str(
            ICP.get_param("omnichannel_bridge.new_contact_as_lead", "False")
        ).lower() not in (
            "1",
            "true",
            "yes",
        ...
        try:
            ...
            self.env["crm.lead"].sudo().create(
                {
                    "name": "[%s] %s"
                    % (provider_label, partner.display_name or "New contact"),
                    "partner_id": partner.id,
                    "description": "Новий контакт через %s. Призначте менеджера."
                    % provider_label,
                    "tag_ids": [],
                }
            )
    ...
    def _omni_telegram_getchat_snapshot(self, chat_id):
        ...
        try:
            ...
            _logger.debug(
                "Telegram getChat failed (network) chat_id=%s", cid, exc_info=True
            )
            return {}
        try:
    ...
    @api.model
    def _omni_telegram_build_inbound_metadata(
        self, from_user, chat, contact, getchat_snapshot
    ):
    ...
    def _omni_process_telegram(self, payload, headers):
        ...
        contact = (
            message.get("contact") if isinstance(message.get("contact"), dict) else {}
        )
    ...
    def _omni_handle_bot_command(self, text, from_user, chat):
        ...
        if cmd == "/stop_bot":
            ...
            self._omni_telegram_send_message(
                chat_id, "🔴 Бот ВИМКНЕНО. Всі відповіді — вручну."
            )
            _logger.warning(
                "Bot stopped via Telegram command by user %s", from_user.get("id")
            )
            ...
            self._omni_telegram_send_message(
                chat_id, "🟢 Бот УВІМКНЕНО. Режим: поза робочими годинами."
            )
            _logger.info(
                "Bot started via Telegram command by user %s", from_user.get("id")
            )
            ...
            _logger.info(
                "Bot restarted via Telegram command by user %s", from_user.get("id")
            )

        elif cmd == "/bot_status":
            enabled = ICP.get_param("omnichannel_bridge.llm_enabled", "False")
            mode = ICP.get_param(
                "omnichannel_bridge.bot_reply_mode", "outside_manager_hours"
            )
            ...
            for change in entry.get("changes", []):
                ...
                for msg in messages:
                    ...
                    if msg_type == "text":
                        ...
                        button_reply = (interactive.get("button_reply") or {}).get(
                            "title"
                        )
    ...
    @api.model
    def omni_send_outbound(
        self, provider, external_thread_id, customer_partner, body_html
    ):
    ...
    def _omni_log_outbound_delivery(
        self, provider, external_thread_id, endpoint, ok, status_code=0, error=""
    ):
        try:
    ...
    def _omni_mask_pii_for_logs(self, text):
        ...
        icp = self.env["ir.config_parameter"].sudo()
        enabled = str(
            icp.get_param("omnichannel_bridge.log_pii_masking", "True")
        ).lower() in (
            "1",
            "true",
            "yes",
    ...
    def _omni_telegram_send_message(self, chat_id, text):
        ...
        try:
            ...
            self._omni_log_outbound_delivery(
                "telegram", chat_id, url, False, 0, "request_exception"
            )
            return
        if not resp.ok:
            _logger.error(
    ...
    def _omni_meta_send_psid(self, psid, text):
        ...
        try:
            ...
            self._omni_log_outbound_delivery(
                "meta", psid, url, False, 0, "request_exception"
            )
            return
        if not resp.ok:
            _logger.error(
    ...
    def _omni_whatsapp_send_to_wa_id(self, wa_id, text):
        ...
        try:
            ...
            self._omni_log_outbound_delivery(
                "whatsapp", wa_id, url, False, 0, "request_exception"
            )
            return
        if not resp.ok:
            _logger.error(
    ...
    def _omni_viber_send_to_user(self, viber_id, text):
        ...
        try:
            ...
            self._omni_log_outbound_delivery(
                "viber", viber_id, url, False, 0, "request_exception"
            )
            return
        if not resp.ok:
            _logger.error(
