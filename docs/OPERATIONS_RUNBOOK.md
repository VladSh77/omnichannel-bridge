...
Symptoms: RPC error when opening a menu or calling a method; traceback shows `KeyError: '<technical model name>'` (often `omni.*` from this module) in `registry[model_name]`.
...
1. Confirm the server’s addons path includes the same `omnichannel_bridge` revision that defines the model (see `models/*.py` and `_name`; runtime smoke lists expected names in `scripts/odoo_runtime_smoke.py` → `_REQUIRED_OMNI_MODELS`).
...
4. Optional: in `odoo shell`, run `scripts/odoo_runtime_smoke.py` `run(env)` — it asserts every `omni.*` model from `_REQUIRED_OMNI_MODELS` is in the registry when the module is installed.
